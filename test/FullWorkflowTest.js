...
contract("Full Workflow Test", accounts => {
    ...
    it("full lifecycle of a tweet, proposition, and voting", async () => {
        ...
        await astrea.addProposition(tweetId,0,0, { from: accounts[0] });

        
        await karmaToken.mint(accounts[1], web3.utils.toWei('100', 'ether'), { from: accounts[0] });
        await karmaToken.mint(accounts[2], web3.utils.toWei('100', 'ether'), { from: accounts[0] });

      
        await astrea.addPlayer(accounts[1]);
        await astrea.addPlayer(accounts[2]);

        await astrea.certifyProposition(tweetId, false, web3.utils.toWei('50', 'ether'), { from: accounts[2] });
        await astrea.voteOnProposition(tweetId, true, web3.utils.toWei('5', 'ether'), { from: accounts[1] });
        

        
        let isDecided = await astrea.isPropositionDecided(tweetId);
        assert(isDecided, "Proposition should be decided based on votes");
